...
export class AuthController {
  static async signup({ username, email, password }) {
    ...
    webpushEmails.push(email);
    //updateEmailStore("webpush", webpushEmails);
    updateEmailStore("email", webpushEmails);
  ...
  static async getAllUsersEmails() {
    try {
      ...
      const emails = users.map((user) => user.email); // Extract only the emails
      return emails; // Return the emails array
    } catch (err) {
      throw new Error("Error retrieving users emails");
    }
  }
